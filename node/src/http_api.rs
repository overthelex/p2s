...
async fn publish_card(
    ...
    let pubkey = match hex::decode(&req.record.pubkey) {
        Ok(b) => b,
        Err(e) => return (StatusCode::BAD_REQUEST, Json(serde_json::json!({"error": format!("invalid pubkey hex: {e}")}))),
    };
    let sig = match hex::decode(&req.sig) {
        Ok(b) => b,
        Err(e) => return (StatusCode::BAD_REQUEST, Json(serde_json::json!({"error": format!("invalid sig hex: {e}")}))),
    };
    let manifest_hash = match hex::decode(&req.record.manifest_hash) {
        Ok(b) => b,
        Err(e) => return (StatusCode::BAD_REQUEST, Json(serde_json::json!({"error": format!("invalid manifest_hash hex: {e}")}))),
    };

    let status = match req.record.status.as_str() {
        "active" => p2s_proto::CardStatus::Active,
        "revoked" => p2s_proto::CardStatus::Revoked,
        _ => return (StatusCode::BAD_REQUEST, Json(serde_json::json!({"error": "status must be 'active' or 'revoked'"}))),
    };
    ...
    let cmd = SwarmCommand::PutRecord {
        key: address.to_vec(),
        value,
        reply: reply_tx,
    };
                ...
                Ok(card) => {
                    let resp = serde_json::json!({
                        "record": {
                            ...
                            "seq": card.record.seq,
                            "status": match card.record.status {
                                p2s_proto::CardStatus::Active => "active",
                                p2s_proto::CardStatus::Revoked => "revoked",
                            },
